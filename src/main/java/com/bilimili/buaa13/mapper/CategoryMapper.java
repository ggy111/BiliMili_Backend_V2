...
import org.apache.ibatis.annotations.Mapper;
import org.apache.ibatis.annotations.Param; //不知道是不是这个类
...
public interface CategoryMapper extends BaseMapper<Category> {
    ...
    // 获取所有分类及其对应的主分类ID
    @Select("SELECT category_name, main_class_id FROM category")
    ...
    // 获取指定主分类下的所有分类名称
    @Select("SELECT category_name FROM category WHERE main_class_id = #{mcId}")
    ...
    // 查找子分类的分类名称
    @Select("SELECT category_name FROM category WHERE sub_class_id = #{scId}")
    ...
    @Select("SELECT main_class_id, sub_class_id, category_name FROM category")
    List<Map<String, String>> findAllCategoryDetails();

    // 获取某个主分类下，所有分类及其子分类名称
    @Select("SELECT category_name FROM category WHERE main_class_id = #{mcId} ORDER BY sub_class_id")
    List<String> findCategoryNamesByMainClassIdOrderedBySubClass(@Param("mcId") String mcId);

    // 获取指定主分类和子分类ID的分类名称
    @Select("SELECT category_name FROM category WHERE main_class_id = #{mcId} AND sub_class_id = #{scId}")
    ...
    // 根据分类ID获取主分类ID和子分类ID
    @Select("SELECT main_class_id, sub_class_id FROM category WHERE id = #{id}")
    ...
    @Select("SELECT sub_class_id, category_name FROM category WHERE main_class_id = #{mcId} ORDER BY sub_class_id")
    List<Map<String, String>> findSubClassListByMainClassIdOrdered(@Param("mcId") String mcId);

    // 获取所有分类的层级结构
    @Select("SELECT main_class_id, sub_class_id, category_name FROM category ORDER BY main_class_id, sub_class_id")
    List<Map<String, String>> findAllCategoryHierarchy();

    // 根据分类名称模糊查询获取分类ID
    @Select("SELECT id FROM category WHERE category_name LIKE CONCAT('%', #{name}, '%')")
    ...
    // 获取分类表中重复的分类名称及其出现次数
    @Select("SELECT category_name, COUNT(*) as occurrence FROM category GROUP BY category_name HAVING COUNT(*) > 1")
